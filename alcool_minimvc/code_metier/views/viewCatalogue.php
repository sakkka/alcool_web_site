...
<div class= "boxe">
     <div class= "inboxe">
        <p>
        <img src = '<?php echo $alcool['image']; ?>' height=120 alt="alcool"/>
        ...
        <form action=panier/addpanier method=post>
         <p>  <INPUT type=hidden name=numproduit value=<?php echo $alcool['Numero_produit'];  ?> >
            ...
            <INPUT class="addpanier" type="submit" <?php if ($alcool['Stock'] <= 0) echo 'disabled'; ?> value="ajouter au panier" /></p>
        </form>
       ...
       <?php $key=count($donnees)-1;
        if (isset($donnees[$key][(count($donnees[$key]) - 1)][0]['ID_u'])){
            if ($donnees[$key][(count($donnees[$key]) - 1)][0]['Root'] == 1){?>
                ...
                <p><input class="addpanier" type="submit" value="Ajouter Stock" /></p>
                </form>
        <?php
            }
        }
